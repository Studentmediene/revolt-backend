...
def get_shows():
    r = requests.get('{base}/programmer/list'.format(base=settings.RR_API_BASE), timeout=29.)
    r.raise_for_status()
...
def get_podcast_url_from_digas_id(digas_id):
    ...
    url = '{base}/url/{digas_id}'.format(base=settings.PODCAST_API_BASE, digas_id=digas_id)
